...
func (qqService *QQService) GetAccessTokenByCode(code string) (other.AccessTokenResponse, error) {
	...
	if err != nil {
		return data, err // 请求失败（如网络错误），返回空数据和错误
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusOK {
		return data, fmt.Errorf("request failed with status code : %d", res.StatusCode)
	}
	byeDate, err := io.ReadAll(res.Body)
	if err != nil {
		return data, err
	}
	err = json.Unmarshal(byeDate, &data)
	if err != nil {
		return data, err
	}
	return data, nil
