<template>
  <div class="forgot-password-form">
    ...
    <!-- 1. :validate-on-rule-change="false"
属性含义：控制 “表单验证规则（rules）发生变化时，是否自动触发表单验证”。
2.hide-required-asterisk
属性含义：隐藏必填项前的 “必填星号（*）” 标记。
3. @keyup.enter="submitForm"
事件含义：监听 “键盘 Enter 键抬起事件”，当用户按下 Enter 键并松开时，触发 submitForm 方法（表单提交逻辑）。
-->
    <el-form ref="forgotPasswordForm" :model="forgotPasswordFormData" :rules="rules" :validate-on-rule-change="false"
      hide-required-asterisk @keyup.enter="submitForm">

      <el-form-item label="邮箱" prop="email">
        <el-input v-model="forgotPasswordFormData.email" size="large" placeholder="请输入邮箱" />
      </el-form-item>
      <el-form-item prop="captcha">
        <div class="captcha">
          <el-input v-model="emailRequest.captcha" placeholder="请输入图片验证码" size="large" maxlength="6" minlength="6" />
          <el-image :src="picPath" alt="" @click="emailVerify" />
          <el-button @click="sendCode">发送验证码</el-button>
        </div>
      </el-form-item>
      <el-form-item label="邮箱验证码" prop="verification_code">
        <el-input v-model="forgotPasswordFormData.verification_code" size="large" placeholder="请输入邮箱验证码" />
      </el-form-item>
      <el-form-item label="密码" prop="new_password">
        <el-input v-model="forgotPasswordFormData.new_password" show-password size="large" type="password"
          placeholder="请输入新密码" />
      </el-form-item>
      <el-form-item label="确认密码">
        <el-input v-model="repeatPassword" show-password size="large" type="password" placeholder="请再次输入新密码" />
        <el-text v-if="forgotPasswordFormData.new_password !== repeatPassword">两次密码不一致！</el-text>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" size="large" @click="submitForm">确定
        </el-button>
      </el-form-item>
    </el-form>
  </div>
