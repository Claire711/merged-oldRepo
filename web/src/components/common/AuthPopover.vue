<template>
  <div class="auth-popover">
    <el-popover v-if="isLoggedIn" width="280" trigger="click">
      <!--trigger="click" 配置需要搭配 #reference 插槽指定触发元素-->
      <template #reference>
    ...
    <el-popover v-else :visible="popoverVisible" width="280">
      <template #reference>
        ...
        <!--这是 layoutStore 中存储的 布尔类型响应式变量，用于控制 “弹出层”（如用户信息弹窗）的显示 / 隐藏-->
      </template>
      <template #default>
        <div class="default" v-click-outside="onClickOutside">
          ...
          <div class="auth-button">
            <div class="button-item">
              ...
              登录
              <el-dialog v-model="loginVisible" width="500" destroy-on-close align-center
            ...
            <div class="button-item">
              ...
              注册
              <el-dialog v-model="registerVisible" width="640" destroy-on-close align-center
            ...
            <div class="button-item">
              <el-button class="forgot-password" icon="Unlock"
                @click="layoutStore.state.forgotPasswordVisible = true" />
              找回密码
              <el-dialog v-model="forgotPasswordVisible" width="570" destroy-on-close align-center
...
//<style scoped> 里的 scoped 是一个样式作用域限定属性，它的核心作用是：让当前 <style> 标签内的样式只对当前组件的元素生效，
//不会污染其他组件或全局样式，避免样式冲突。
.auth-popover {
  //控制组件处于 “禁用状态” 时的文本颜色
