<template>
    <div class="user-star">
        ...
        <el-table :data="articleLikesListData">
        ...
        <el-pagination :current-page="page" :page-size="page_size" :page-sizes="[10, 30, 40, 50, 100]" :total="total"
...
//useRoute() 后面的 () 是 函数调用运算符，作用是执行 useRoute 这个函数，并获取它的返回值。
...
const getArticleLikesListData = async () => {
    ...
    const res = await articleLikesList(articleLikesListRequest)
    if (res.code === 0) {
        articleLikesListData.value = res.data.list
...
//{ immediate: true } 的作用：初始化时立即执行一次

//nextTick 的作用是：将回调函数延迟到下一次 DOM 更新循环结束后执行。
nextTick(() => {
