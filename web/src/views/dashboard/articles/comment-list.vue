  ...
  <div class="comment-list">
    <div class="title">
      ...
      <el-button-group>
        ...
        <el-dialog v-model="commentBulkDeleteVisible" width="500" align-center destroy-on-close>
          <template #header>
            删除评论
          </template>
    ...
    <el-table ref="multipleCommentTableRef" :data="commentTableData">
      ...
      <!-- 在表格操作列中设置 commentInfo -->
      <el-table-column label="操作" width="100">
        <template #default="scope: { row: Comment, column: any, $index: number }">
          <el-button type="danger" @click="commentDeleteVisible = true; commentInfo = scope.row">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="commentDeleteVisible" width="500" align-center destroy-on-close>
      <template #header>
        删除评论
      </template>
...
const handleIdsToDelete = () => {
  idsToDelete = []
  // 通过 multipleCommentTableRef 获取选中的行数据
//multipleCommentTableRef : handleIdsToDelete() 方法中获取选中行的id，并将id存入idsToDelete数组中
